import requests
import traceback
import re
import os

# 环境变量配置
CF_API_TOKEN = os.environ["CF_API_TOKEN"]
CF_ZONE_ID = os.environ["CF_ZONE_ID"]
CF_DNS_NAME = os.environ["CF_DNS_NAME"]
HEADERS = {
'Authorization': f'Bearer {CF_API_TOKEN}',
'Content-Type': 'application/json'
}

def extract_ipv4s(text):
"""从文本中提取合法的 IPv4 地址"""
pattern = r'(?:\d{1,3}\.){3}\d{1,3}'
candidates = re.findall(pattern, text)
seen = set()
result = []
for ip in candidates:
parts = ip.split('.')
if len(parts) == 4 and all(p.isdigit() and 0 <= int(p) <= 255 for p in parts):
if ip not in seen:
seen.add(ip)
result.append(ip)
return result

def get_ips_from_url(url):
"""从指定 URL 获取 IP"""
try:
r = requests.get(url, timeout=15)
r.raise_for_status()
ips = extract_ipv4s(r.text)
return ips
except Exception:
traceback.print_exc()
return []

def list_a_records(name):
"""列出当前 CF 上的 A 记录"""
url = f'https://api.cloudflare.com/client/v4/zones/{CF_ZONE_ID}/dns_records'
params = {'type': 'A', 'name': name, 'per_page': 100}
try:
r = requests.get(url, headers=HEADERS, params=params, timeout=10)
r.raise_for_status()
return r.json().get("result", [])
except Exception:
traceback.print_exc()
return []

def delete_dns_record(record_id):
"""删除单个 DNS 记录"""
url = f'https://api.cloudflare.com/client/v4/zones/{CF_ZONE_ID}/dns_records/{record_id}'
try:
r = requests.delete(url, headers=HEADERS, timeout=10)
r.raise_for_status()
print(f"Deleted DNS record {record_id}")
return True
except Exception:
traceback.print_exc()
return False

def delete_all_a_records(name):
"""删除指定域名的所有 A 记录"""
records = list_a_records(name)
ok = True
for rec in records:
rid = rec.get('id')
if rid:
ok = delete_dns_record(rid) and ok
return ok

def create_dns_record(name, ip):
"""创建新的 DNS A 记录"""
url = f'https://api.cloudflare.com/client/v4/zones/{CF_ZONE_ID}/dns_records'
data = {'type': 'A', 'name': name, 'content': ip}
try:
r = requests.post(url, headers=HEADERS, json=data, timeout=10)
r.raise_for_status()
print(f"Created DNS A record {ip}")
return f"ip:{ip} added successfully"
except Exception:
traceback.print_exc()
return f"ip:{ip} add failed"

def main():
target_url = 'https://ip.164746.xyz/ipTop.html'

# 1. 从 URL 获取 IP
fetched_ips = get_ips_from_url(target_url)
print(f"Fetched IPs from URL: {fetched_ips}")

# 如果没有获取到 IP，则退出
if not fetched_ips:
print("No IPs found; exiting")
return

# 去重
all_ips = list(dict.fromkeys(fetched_ips))

# 2. 处理数量限制 (CF_MAX_RECORDS)
max_records = os.getenv('CF_MAX_RECORDS')
if max_records and max_records.isdigit():
all_ips = all_ips[:int(max_records)]

# 3. 删除旧记录
if not delete_all_a_records(CF_DNS_NAME):
print("Warning: delete old records failed; continue")

# 4. 添加新记录
results = [create_dns_record(CF_DNS_NAME, ip) for ip in all_ips]
for res in results:
print(res)

if __name__ == '__main__':
main()
